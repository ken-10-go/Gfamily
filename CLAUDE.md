...
**家 = 婚姻の線を外して親子だけでたどった連結成分**（`src/features/tree-view/houses.ts` の
`bloodGroups`）。婚姻は家と家を「つなぐ」ものなので、たどる対象から外すと家の輪郭が出る。
...
まだ配置には使っていない（次の段）。X座標は今も世代ごとに左から詰めるだけなので、
人数が増えると姻族が空いている場所へ押し出されて血縁のまとまりが散らばる。
