...
### キャッシュの設定（`firebase.json`）

画面の入口となる HTML は毎回取り直させ、ハッシュ付きの資産（`/assets/**`）だけを長期キャッシュにしています。

`headers` の `source` は「リクエストされたパス」に対して照合され、rewrite 後のファイル名では
ありません。以前は `/index.html` にだけ `no-cache` を指定していたため、**トップページ（`/`）が
既定のキャッシュ（1時間ほど）で返り、デプロイしても古い画面が出続ける**ことがありました。
いまは `**` を対象にして、`/` を含むすべての入口を毎回取り直させています。
...
### ログインの用意（Firebase コンソール）

家族が2通りの入り方を選べるようにするため、Authentication で次を有効にします。

- **Google** — Google アカウントを持っている人向け
- **メール／パスワード** — 持っていない人が、自分で決めて登録する

「メール／パスワード」を無効にしたままだと、招待画面の登録が失敗します。
アカウントを作っただけでは何も見えません（どの家系図にも属していないため）。
参加できるのは招待リンクを受け取った人だけです。
